...
def filter_to_gamma_T8(epoch_directory, T8_gamma_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['T8']).filter(30, 45)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_T8_gamma-band.fif')
        filtered_output_path = os.path.join(T8_gamma_directory, filtered_filename)
