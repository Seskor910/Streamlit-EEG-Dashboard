...
def filter_to_beta_F3(epoch_directory, F3_beta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['F3']).filter(12, 30)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_F3_beta-band.fif')
        filtered_output_path = os.path.join(F3_beta_directory, filtered_filename)
