...
def filter_to_delta_F3(epoch_directory, F3_delta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['F3']).filter(0.1, 3)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_F3_delta-band.fif')
        filtered_output_path = os.path.join(F3_delta_directory, filtered_filename)
