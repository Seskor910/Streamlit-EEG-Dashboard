...
def filter_to_theta_TP7(epoch_directory, TP7_theta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['TP7']).filter(4, 7)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_TP7_theta-band.fif')
        filtered_output_path = os.path.join(TP7_theta_directory, filtered_filename)
