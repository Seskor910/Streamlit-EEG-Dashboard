...
def filter_to_theta_FT7(epoch_directory, FT7_theta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['FT7']).filter(4, 7)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_FT7_theta-band.fif')
        filtered_output_path = os.path.join(FT7_theta_directory, filtered_filename)
