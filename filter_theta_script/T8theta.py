...
def filter_to_theta_T8(epoch_directory, T8_theta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['T8']).filter(4, 7)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_T8_theta-band.fif')
        filtered_output_path = os.path.join(T8_theta_directory, filtered_filename)
