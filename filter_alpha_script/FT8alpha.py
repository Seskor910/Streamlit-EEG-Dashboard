...
def filter_to_alpha_FT8(epoch_directory, FT8_alpha_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['FT8']).filter(8, 12)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_FT8_alpha-band.fif')
        filtered_output_path = os.path.join(FT8_alpha_directory, filtered_filename)
