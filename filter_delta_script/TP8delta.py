...
def filter_to_delta_TP8(epoch_directory, TP8_delta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_TP8_delta-band.fif')
        filtered_output_path = os.path.join(TP8_delta_directory, filtered_filename)
