...
def filter_to_beta_TP7(epoch_directory, TP7_beta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['TP7']).filter(12, 30)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_TP7_beta-band.fif')
        filtered_output_path = os.path.join(TP7_beta_directory, filtered_filename)
