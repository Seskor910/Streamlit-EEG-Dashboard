...
def filter_to_beta_T8(epoch_directory, T8_beta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['T8']).filter(12, 30)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_T8_beta-band.fif')
        filtered_output_path = os.path.join(T8_beta_directory, filtered_filename)
