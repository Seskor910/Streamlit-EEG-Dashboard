...
def filter_to_alpha_FT7(epoch_directory, FT7_alpha_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['FT7']).filter(8, 12)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_FT7_alpha-band.fif')
        filtered_output_path = os.path.join(FT7_alpha_directory, filtered_filename)
