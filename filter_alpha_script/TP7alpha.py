...
def filter_to_alpha_TP7(epoch_directory, TP7_alpha_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['TP7']).filter(8, 12)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_TP7_alpha-band.fif')
        filtered_output_path = os.path.join(TP7_alpha_directory, filtered_filename)
