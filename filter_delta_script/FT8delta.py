...
def filter_to_delta_FT8(epoch_directory, FT8_delta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['FT8']).filter(0.1, 3)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_FT8_delta-band.fif')
        filtered_output_path = os.path.join(FT8_delta_directory, filtered_filename)
