...
def filter_to_alpha_F3(epoch_directory, F3_alpha_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['F3']).filter(8, 12)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_F3_alpha-band.fif')
        filtered_output_path = os.path.join(F3_alpha_directory, filtered_filename)
