...
def filter_to_delta_TP7(epoch_directory, TP7_delta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['TP7']).filter(0.1, 3)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_TP7_delta-band.fif')
        filtered_output_path = os.path.join(TP7_delta_directory, filtered_filename)
