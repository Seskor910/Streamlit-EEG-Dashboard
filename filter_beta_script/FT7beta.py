...
def filter_to_beta_FT7(epoch_directory, FT7_beta_directory):
    ...
    for fif_file_path in fif_files:
        ...
        raw.pick_channels(['FT7']).filter(12, 30)

        # Saving the filtered signal
        filtered_filename = os.path.basename(fif_file_path).replace('.fif', '_FT7_beta-band.fif')
        filtered_output_path = os.path.join(FT7_beta_directory, filtered_filename)
